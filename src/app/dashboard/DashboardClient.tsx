...
interface DbAlertItem {
  ...
  severity: string;
  createdAt: string;
  // Prisma 는 미설정 컬럼을 null 로 준다 (undefined 아님) — 서버 컴포넌트에서
  // 그대로 내려오므로 null 을 허용해야 한다.
...
/**
 * DB 알림 행을 화면용 알림으로 변환.
 * metadata.eventAt (공시 접수일 등) 이 있으면 표시·정렬에 우선 사용하고,
 * 없으면 createdAt (DB 저장 시각) 으로 폴백.
 */
function toUiAlert(a: DbAlertItem): UiAlert {
...
/**
 * 대시보드. positions/alerts 는 서버 컴포넌트(page.tsx)가 DB 에서 직접 읽어
 * props 로 내려주므로 첫 화면에 요약 타일·풋 카드·알림이 이미 채워져 있다.
 * 클라이언트는 느린 외부 데이터(네이버 시세, 뉴스/공시 피드)만 이어서 붙인다.
 */
export default function DashboardClient({
  initialPositions,
  initialAlerts,
}: {
  initialPositions: PositionSlim[];
  initialAlerts: DbAlertItem[];
}) {
  const [stats, setStats] = useState<DashboardStats | null>(() =>
    initialPositions.length > 0
      ? { totalPositions: initialPositions.length, topRisers: [], topFallers: [] }
      : null,
  );
  // 서버가 이미 요약을 채워줬으면 첫 렌더에서 스피너를 띄우지 않는다.
  ...
  const [alertList, setAlertList] = useState<UiAlert[]>(() =>
    initialAlerts.map(toUiAlert),
  );
  const [positions, setPositions] = useState<PositionSlim[]>(initialPositions);
  ...
  async function loadDashboard(opts?: { forceRefresh?: boolean }) {
    ...
    //
    // positions/stats/alertList 는 더 이상 캐시에서 복원하지 않는다 — 서버 컴포넌트가
    // 요청 시점에 DB 에서 읽은 최신값을 이미 초기 state 로 넣어줬으므로, 이전 방문의
    // 캐시로 덮으면 오히려 옛 데이터가 보인다.
    // 캐시는 서버가 줄 수 없는 것(외부 API 유래: 뉴스/공시 피드, 네이버 시세)에만 쓴다.
    ...
    try {
      ...
      // 첫 로드에서는 서버가 내려준 positions/alerts 를 그대로 쓰고, 느린 외부
      // 데이터(시세)만 따로 받는다 — init 왕복이 통째로 사라진다.
      // 새로고침 버튼(forceRefresh)일 때만 DB 까지 다시 읽는다.
      const useServerData = !forceRefresh && initialPositions.length > 0;

      let positions: PositionSlim[];
      let dbAlertRes: DbAlertItem[];
      let quotes: Record<string, { price?: number; changeRate?: number }>;

      if (useServerData) {
        positions = initialPositions;
        dbAlertRes = initialAlerts;
        const tickerList = Array.from(
          new Set(
            initialPositions
              .filter((p) => p.isActive && /^\d{6}$/.test(p.underlyingTicker))
              .map((p) => p.underlyingTicker),
          ),
        );
        const priceRes = tickerList.length
          ? await fetch(
              `/api/prices?tickers=${tickerList.join(",")}&save=false&t=${Date.now()}`,
              { cache: "no-store" },
            )
              .then((r) => r.json())
              .catch(() => ({ quotes: {} }))
      ...
      const dbAlerts: UiAlert[] = dbAlertRes.map(toUiAlert);
        ...
        .then((feedRes) => {
          ...
          try {
            sessionStorage.setItem(
              "dashboardCache",
              JSON.stringify({
                // 외부 API 유래 데이터만 캐시한다. positions/stats/alertList 는
                // 서버 렌더가 매 요청 최신값을 주므로 캐시할 이유가 없다.
                livePrices: priceMap,
                news: feedData.news ?? [],
