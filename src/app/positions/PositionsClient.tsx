...
/**
 * 보유종목 표. 초기 데이터는 서버 컴포넌트(page.tsx)가 DB 에서 직접 읽어
 * props 로 내려주므로, 첫 화면에 이미 목록이 채워져 있다.
 * 클라이언트는 이후 시세 갱신과 정렬·필터 상호작용만 담당한다.
 */
export default function PositionsClient({
  ...
  // 서버가 이미 목록을 채워줬으므로 첫 렌더에서 스피너를 띄우지 않는다.
  ...
  useEffect(() => {
    // 서버가 목록을 이미 내려줬으므로 sessionStorage hydrate 도, 목록 재조회도
    // 필요 없다. 시세만 바로 붙이면 된다.
    if (initialPositions.length > 0) {
      refreshLivePricesByTickers(initialPositions.map((p) => p.underlyingTicker));
      try {
        sessionStorage.setItem("positionsList", JSON.stringify(initialPositions));
      } catch {
        // storage full 등 무시
      }
      return;
    ...
    // 서버 데이터가 비어있는 경우 (첫 등록 직후 등) 만 클라이언트에서 한 번 조회
    ...
    // initialPositions 는 서버 렌더마다 새 배열이지만 마운트 시 1회만 돌면 된다.
